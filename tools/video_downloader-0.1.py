...
class UniversalVideoSpider:
        ...
        with sync_playwright() as p:
            ...
            def handle_response(response):
                try:
                    ...
                    if any(x in url for x in ["ad.", "/ad/", "adv", "blank", "test", "preview"]):
                        return
            ...
            try:
                ...
                self.log(f"[!] 页面加载异常或超时 (可能是正常的反爬拦截): {e}")
            finally:
                browser.close()

        if found_m3u8:
            final_url = found_m3u8[-1]
            self.log(f"[+] 决策结果: 优先使用最后捕获的 M3U8 流。")
            return final_url
        elif found_mp4:
            final_url = found_mp4[-1]
            self.log(f"[+] 决策结果: 未发现 M3U8，降级使用捕获到的 MP4。")
            return final_url
    ...
    async def _download_m3u8(self, m3u8_url: str, output_filename: str):
        ...
        if playlist.keys and playlist.keys[0]:
            ...
            self.log("[+] 检测到加密，已加载最新的 cryptography AES 引擎。")

        video_temp_dir = os.path.join(self.temp_dir, output_filename)
        os.makedirs(video_temp_dir, exist_ok=True)

        ts_files_list = []
        self.log(f"[*] 共发现 {len(playlist.segments)} 个切片，准备下载...")
        ...
        self.log("[+] 切片列队处理完成，开始使用 FFmpeg 合并并转码为 MP4...")
        final_mp4_path = os.path.join(self.output_dir, f"{output_filename}.mp4")
        self._merge_with_ffmpeg(ts_files_list, final_mp4_path)
    ...
    def _merge_with_ffmpeg(self, ts_files: list, output_mp4: str):
        valid_ts_files = [ts for ts in ts_files if os.path.exists(ts)]

        if not valid_ts_files:
            self.log("[-] 没有任何有效的 TS 切片被下载，合并任务中止！")
            return

        if len(valid_ts_files) < len(ts_files):
            self.log(f"[!] 警告: 有 {len(ts_files) - len(valid_ts_files)} 个切片缺失。仍将合并现有分段。")
        ...
        try:
            self.log("[*] 正在执行 FFmpeg 底层无损合并，请稍候...")
            startupinfo = None
            if os.name == 'nt':
...
class VideoDownloaderTool(QWidget):
    ...
    def __init__(self):
        ...
        # 【修改点 1】：去掉写死的 setStyleSheet，加入 Emoji 区分状态
    ...
    # 【修改点 2】：去掉切换时的 setStyleSheet 颜色改变，仅改变文本
